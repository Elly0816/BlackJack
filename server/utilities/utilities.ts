...
export function getNumberFromCard(
  card: ReturnType<Card["getCardDetails"]>["face"],
): number {
  const cardNumber = card.toUpperCase();
  if (cardNumber.startsWith("A")) return 1;
  if (
    cardNumber.startsWith("J") ||
    cardNumber.startsWith("Q") ||
    cardNumber.startsWith("K")
  )
    return 10;
...
export async function parseDeck(): Promise<Card[]> {
  ...
  try {
    ...
    let cardsFromFile: string | string[] = (
      await fs.readFile("./text.txt", { encoding: "utf8" })
    )
      .replace("]", "")
      .replace("[", "")
      .split(",")
