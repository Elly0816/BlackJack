...
const { cards, gameState, gameRoom, gamePlayer } = require("./gameDetails2");
  ...
  socket.on("search", () => {
    if (gameRooms.length > 0) {
      for (const gameRoom of gameRooms) {
        if (
          gameRoom.numOfPlayers < 2 &&
          gameRoom.roomSocket.id !== reverseString(socket.id)
        ) {
      ...
      //There is no room in the game Rooms array
